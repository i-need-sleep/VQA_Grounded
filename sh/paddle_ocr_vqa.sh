#!/bin/bash
#SBATCH --job-name=paddle_ocr_vqa   # 任务名
...
module load anaconda3 cuda/10.2 cudnn/7.6.5              # 加载anaconda (load virtual env for training)
...
echo "START"               # 输出起始信息
source deactivate
source /gpfsnyu/packages/anaconda3/5.2.0/bin/activate paddle          # 调用 virtual env
python -u paddle_ocr_vqa.py 
echo "FINISH"                       # 输出起始信息
